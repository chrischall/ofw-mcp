...
OFW_ATTACHMENTS_DIR       Optional. Where ofw_download_attachment writes (default ~/Downloads/ofw-mcp). Every disk write is confined to it: a saveTo outside it (after resolving symlinks) is refused, and an existing file is replaced only with force:true
OFW_UPLOAD_DIR            Optional. The only directory ofw_upload_attachment may read from (default: OFW_ATTACHMENTS_DIR). Paths outside it after resolving symlinks, hidden files/dirs, and files over 25 MiB are refused — an upload discloses the file to OFW, so an injected "upload ~/.ssh/…" must be impossible
...
OFW_WRITE_MODE            Optional. "none" = no write tools registered; "drafts" = draft-level writes only (ofw_save_draft, ofw_delete_draft, ofw_upload_attachment with shareClass PRIVATE only, since SHARED is co-parent-visible at once — never send or calendar/expense/journal writes); "all" = everything (default). Unrecognized values fail closed to "none". Structural gate: gated tools are not registered at all, so no host setting or injected instruction can invoke them.
...
- `ofw_send_message` and `ofw_save_draft` resolve `replyToId` to the latest sent reply in the same chain via the cache (transparency note included in the response when rewritten); after the OFW POST succeeds they immediately `GET /pub/v3/messages/{id}` to repopulate the cache from authoritative state. (OFW's POST response is minimal — typically `{entityId: X}` — so we use the detail GET as the source of truth.) The re-fetched detail is compared to the posted subject/body (`verifyWriteLanded` in `tools/_shared.ts`, containment not equality — replies get the original appended); a `WARNING` is included in the response when the write can't be confirmed. If the POST response carries no id, `ofw_send_message` does NOT delete the source draft (the send is unconfirmed). A POST that fails WITHOUT a definitive answer (timeout, dropped connection, 5xx, 408), or one OFW accepted whose confirming GET then failed, surfaces from `postMessageAndRefetch` as `UnconfirmedWriteError`; `ofw_send_message` turns it into a `SEND_UNCONFIRMED` result that says the message may have gone out and not to retry blindly, and keeps the draft. Only a definitive 4xx rejection (or a repeated 429) still fails as a plain error, because then nothing was sent.
